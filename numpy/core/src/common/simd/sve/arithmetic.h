...
#define NPYV_IMPL_SVE_DIVC_U(WIDTH)					\
  NPY_FINLINE npyv_u ## WIDTH npyv_divc_u ## WIDTH (npyv_u ## WIDTH a, const npyv_u ## WIDTH ## x3 divisor) \
  { \
...
NPY_FINLINE npyv_s ## WIDTH npyv_divc_s ## WIDTH (npyv_s ## WIDTH a, const npyv_s ## WIDTH ## x3 divisor) \
{ \
  const svbool_t mask_all = svptrue_b ## WIDTH (); \
  const svuint ## WIDTH ## _t shf1 = svreinterpret_u ## WIDTH ## _s ## WIDTH (svget3_s ## WIDTH (divisor, 1)); \
    const svint ## WIDTH ## _t dsign = svget3_s ## WIDTH (divisor, 2); \
    svint ## WIDTH ## _t mulhi = svmulh_s ## WIDTH ## _x(mask_all, a, svget3_s ## WIDTH (divisor, 0)); \
    svint ## WIDTH ## _t q = svasr_s ## WIDTH ## _x(mask_all, svadd_s ## WIDTH ## _x(mask_all, a, mulhi), shf1); \
    q = svsub_s ## WIDTH ## _x(mask_all, q, svasr_n_s ## WIDTH ## _x(mask_all, a, WIDTH - 1)); \
    q = svsub_s ## WIDTH ## _x(mask_all, sveor_s ## WIDTH ## _x(mask_all, q, dsign), dsign); \
    return q; \
}
